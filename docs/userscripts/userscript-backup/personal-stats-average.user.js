...
const showAverages = (startDate) => {
  ...
  for (const textElement of textElements) {
    ...
    const data = personalStats.data[
      Object.keys(personalStats.definitions).filter(
        (key) => personalStats.definitions[key] == statName
      )[0]
    ].filter(
      (user) =>
        user.uid ==
        Object.keys(personalStats.definitions).filter(
          (key) => personalStats.definitions[key] == userName
        )[0]
    )[0].data;
    ...
    const startIndex = startDate
      ? data.findIndex((e) => {
          return (
            new Date(e.time * 1000).toISOString().substring(0, 10) == startDate
          );
        })
      : data.length - 1;
    const totalDays =
      (new Date(data[0].time * 1000) - new Date(data[startIndex].time * 1000)) /
      (86400 * 1000);
    ...
    textElement.textContent =
      textElement.textContent.split(")")[0] + `) ${headerText}`;
  ...
  for (const textElement of textElements) {
    ...
    gNode
      .querySelector("path")
      .setAttribute("d", `M${x},${y + 7}L${x + 24},${y + 7}`);
    ...
    if (mutationEntry.removedNodes) {
      for (const removedNode of mutationEntry.removedNodes) {
        if (
          ...
          removedNode.querySelector("p[class^='date']")
        )
          showAverages();
      }
    }
